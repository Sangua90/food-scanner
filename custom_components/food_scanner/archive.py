...
def _is_expired(item: dict[str, Any]) -> bool:
    ...
    if not raw:
        return False
    try:
        return date.fromisoformat(str(raw)) < dt_util.now().date()
    except ValueError:
        return False
...
class FoodArchive:
    ...
    async def async_load(self) -> None:
        ...
        for raw in raw_items:
            ...
            match = next((x for x in merged if _same_lot(x, item, item.get("location"))), None)
            if match is None:
                merged.append(item)
            else:
    ...
    async def async_add(self, food: dict[str, Any], location: str | None) -> tuple[dict[str, Any], bool, int]:
        ...
        async with self._lock:
            existing = next((x for x in self._items if _same_lot(x, normalized_food, location)), None)
            if existing is not None:
                ...
                existing["category"] = food.get("category") or existing.get("category") or "Altro"
                existing["product_image_url"] = food.get("product_image_url") or existing.get("product_image_url")
                existing["barcode_source"] = food.get("barcode_source") or existing.get("barcode_source")
                ...
                item = {
                    ...
                    "category": food.get("category") or "Altro",
                    "product_image_url": food.get("product_image_url"),
                    "barcode_source": food.get("barcode_source"),
                    "location": location,
    ...
    async def async_add_manual(self, data: dict[str, Any]) -> tuple[dict[str, Any], bool, int]:
        ...
        food = {
            ...
            "confidence": None,
            "category": str(data.get("category") or "Altro").strip() or "Altro",
            "location": location,
        ...
        async with self._lock:
            existing = next((x for x in self._items if _same_lot(x, food, location)), None)
            if existing is not None:
                existing["stock_units"] = _stock_units(existing) + stock_units
                existing["updated_at"] = now
                result, created = dict(existing), False
            else:
                item = {"id": uuid.uuid4().hex, **food, "stock_units": stock_units, "added_at": now, "updated_at": now}
                self._items.append(item)
                result, created = dict(item), True
            await self._async_save()
        self._update_summary_sensors()
    ...
    async def async_remove(self, product_id: str) -> bool:
        ...
        async with self._lock:
            ...
            self._items = [x for x in self._items if x.get("id") != product_id]
            await self._async_save()
        event_type = "expired" if _is_expired(removed_item) else "removed"
        await get_history(self.hass).async_record(event_type, removed_item, _stock_units(removed_item))
        self._update_summary_sensors()
        return True
    ...
    async def async_update_item(self, product_id: str, changes: dict[str, Any]) -> dict[str, Any] | None:
        async with self._lock:
            item = next((x for x in self._items if x.get("id") == product_id), None)
            if item is None:
                return None
            for field in EDITABLE_TEXT_FIELDS:
                ...
                item[field] = str(value).strip() or None if value is not None else None
            ...
            item["updated_at"] = dt_util.utcnow().isoformat()
            duplicate = next((other for other in self._items if other is not item and _same_lot(other, item, item.get("location"))), None)
    ...
    def items_sorted(self, sort: str = "expiry", location: str | None = None, search: str | None = None) -> list[dict[str, Any]]:
        items = self.items()
        if location in VALID_LOCATIONS:
            items = [x for x in items if x.get("location") == location]
        if search:
            needle = _norm(search)
            items = [x for x in items if needle in _norm(x.get("product_name")) or needle in _norm(x.get("brand")) or needle in _norm(x.get("barcode")) or needle in _norm(x.get("category"))]
        if sort == "name":
            key = lambda x: (_norm(x.get("product_name")), _norm(x.get("brand")), x.get("expiry_date") or "9999-12-31")
        elif sort == "added":
            key = lambda x: (x.get("added_at") or "",)
        else:
            key = lambda x: (x.get("expiry_date") is None, x.get("expiry_date") or "9999-12-31", _norm(x.get("product_name")))
    ...
    def summary(self) -> dict[str, Any]:
        ...
        for item in self._items:
            ...
            counts[location if location in VALID_LOCATIONS else "senza_posizione"] += units
            categories[str(item.get("category") or "Altro")] += units
        next_item = next((x for x in self.items_sorted_by_expiry() if x.get("expiry_date")), None)
        return {
            "total_units": total_units,
            "lots": len(self._items),
            **counts,
            "next_expiry": dict(next_item) if next_item else None,
            "categories": [{"category": name, "units": units} for name, units in categories.most_common()],
        }
    ...
    def _update_summary_sensors(self) -> None:
        ...
        self.hass.states.async_set(
            ...
            summary["total_units"],
            {"friendly_name": "Food Scanner - Unità in magazzino", "lots": summary["lots"], "frigo": summary["frigo"], "freezer": summary["freezer"], "dispensa": summary["dispensa"], "senza_posizione": summary["senza_posizione"]},
        )
        if next_item:
            self.hass.states.async_set(
                "sensor.food_scanner_next_expiry",
                next_item["expiry_date"],
                {"friendly_name": "Food Scanner - Prossima scadenza", "product_id": next_item.get("id"), "product_name": next_item.get("product_name"), "brand": next_item.get("brand"), "location": next_item.get("location"), "expiry_type": next_item.get("expiry_type"), "stock_units": _stock_units(next_item), "unit_name": next_item.get("unit_name"), "category": next_item.get("category")},
            )
        else:
            self.hass.states.async_set("sensor.food_scanner_next_expiry", "unknown", {"friendly_name": "Food Scanner - Prossima scadenza"})
