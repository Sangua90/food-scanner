...
async def async_setup_entry(hass:HomeAssistant,entry:ConfigEntry)->bool:
 ...
 q=get_review_queue(hass)
 if not runtime.get("review_queue_loaded"):await q.async_load();runtime["review_queue_loaded"]=True
 history=get_history(hass)
 if not runtime.get("history_loaded"):await history.async_load();runtime["history_loaded"]=True
 consumables=get_consumables(hass)
 if not runtime.get("consumables_loaded"):await consumables.async_load();runtime["consumables_loaded"]=True
 await async_setup_services(hass)
 for key,view in (("upload_view_registered",FoodScannerUploadView),("archive_view_registered",FoodScannerArchiveView),("dashboard_scan_view_registered",FoodScannerDashboardScanView),("consumables_view_registered",FoodScannerConsumablesView)):
...
async def async_unload_entry(hass:HomeAssistant,entry:ConfigEntry)->bool:
 hass.data.get(DOMAIN,{}).pop(entry.entry_id,None);runtime=hass.data.setdefault(RUNTIME_KEY,{})
 ...
 runtime["panel_registered"]=False;return True
