...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if not runtime.get("upload_view_registered"):
        hass.http.register_view(FoodScannerUploadView); runtime["upload_view_registered"] = True
    if not runtime.get("archive_view_registered"):
        hass.http.register_view(FoodScannerArchiveView); runtime["archive_view_registered"] = True
    if not runtime.get("export_view_registered"):
        hass.http.register_view(FoodScannerExportView); hass.http.register_view(FoodScannerExportDataView); runtime["export_view_registered"] = True
    if not runtime.get("panel_static_registered"):
        await hass.http.async_register_static_paths([StaticPathConfig(PANEL_STATIC_URL, str(Path(__file__).parent / "www"), cache_headers=False)])
        runtime["panel_static_registered"] = True
    if not frontend.async_panel_exists(hass, PANEL_URL_PATH):
        await panel_custom.async_register_panel(hass=hass, frontend_url_path=PANEL_URL_PATH, webcomponent_name="food-scanner-panel", sidebar_title="Food Scanner", sidebar_icon="mdi:food-apple-outline", module_url=f"{PANEL_STATIC_URL}/panel_boot.js?v={PANEL_VERSION}", require_admin=False)
    runtime["panel_registered"] = True
    old_notifier = runtime.pop("expiry_notifier", None)
    if old_notifier is not None: await old_notifier.async_unload()
    notifier = ExpiryNotifier(hass, entry); await notifier.async_setup(); runtime["expiry_notifier"] = notifier
    return True
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if notifier is not None: await notifier.async_unload()
    if frontend.async_panel_exists(hass, PANEL_URL_PATH): frontend.async_remove_panel(hass, PANEL_URL_PATH)
