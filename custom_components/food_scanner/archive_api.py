...
def _settings(entry) -> dict[str, Any]:
    return {
        ...
        "expiry_notify_days": entry.options.get(
            CONF_EXPIRY_NOTIFY_DAYS, DEFAULT_EXPIRY_NOTIFY_DAYS
        ),
        "expiry_notify_service": entry.options.get(
...
class FoodScannerArchiveView(HomeAssistantView):
    """API autenticata per magazzino, verifiche e impostazioni Food Scanner."""
        ...
        if action == "add_manual":
            ...
            if not isinstance(changes, dict):
                return self.json_message(
                    "Dati prodotto mancanti o non validi",
        ...
        if action == "update_settings":
            ...
            if "expiry_notify_days" in data:
                try:
                    ...
                    return self.json_message(
                        "Giorni di preavviso non validi",
                        status_code=HTTPStatus.BAD_REQUEST,
                    )
                if days < 0 or days > 365:
                    return self.json_message(
                        "I giorni di preavviso devono essere tra 0 e 365",
                        status_code=HTTPStatus.BAD_REQUEST,
                    )
                current[CONF_EXPIRY_NOTIFY_DAYS] = days
            if "expiry_notify_service" in data:
                current[CONF_EXPIRY_NOTIFY_SERVICE] = str(
                    data.get("expiry_notify_service") or ""
                ).strip()
            if "model" in data:
                model = str(data.get("model") or "").strip()
                if not model:
                    return self.json_message(
                        "Il modello Gemini non può essere vuoto",
                        status_code=HTTPStatus.BAD_REQUEST,
                    )
